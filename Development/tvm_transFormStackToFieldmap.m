...
transformFile =            	fullfile(subjectDirectory, tvm_getOption(configuration, 'i_TransformStack'));
    %no default
fieldMapFile =            	fullfile(subjectDirectory, tvm_getOption(configuration, 'o_FieldMap'));
    %no default
...
fieldMap = cell(1, length(wSurface)); 
...
for hemisphere = 1:length(wSurface)
    ...
    fieldMap{hemisphere} = bsxfun(@times, median(fieldMaps, 5), maskVolume{hemisphere});
end
fieldMap = plus(fieldMap{:}) ./ repmat(plus(maskVolume{:}), [1, 1, 1, 3]);

voxelSize = sqrt(sum(referenceVolume.mat(1:3, 1:3) .^ 2));
filterWidth = [6, 6, 6];
for i = 1:size(fieldMap, 4)
    fieldMap(:, :, :, i) = smoothVolume(fieldMap(:, :, :, i), voxelSize, filterWidth);
end
referenceVolume.pinfo = [1, 0, 352]';
referenceVolume.dt = [16, 0];
tvm_write4D(referenceVolume, fieldMap, fieldMapFile);
