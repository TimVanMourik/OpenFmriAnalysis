function tvm_useFlirt(configuration)
% TVM_REGISTERVOLUMES 
...
moveFile =         fullfile(subjectDirectory, tvm_getOption(configuration, 'i_MoveVolume'));
    %no default
fieldMap =      fullfile(subjectDirectory, tvm_getOption(configuration, 'i_FieldMap'));
    %no default
echoSpacing =              tvm_getOption(configuration, 'i_EchoSpacing', []);
    %no default
...
unixCommand = [unixCommand 'flirt -in ' moveFile ' --ref ' referenceFile ' -fieldmap ' fieldMap ' -pedir y -echospacing ' num2str(echoSpacing) ';'];
unix(unixCommand);
