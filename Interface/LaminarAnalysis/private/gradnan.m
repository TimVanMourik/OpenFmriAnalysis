function varargout = gradnan(f,varargin)
...
%   direction is assumed to be one. When F is a vector, DF = GRADNAN(F)
%   is the 1-D gradient.
%
%   [FX,FY] = GRADNAN(F,H), where H is a scalar, uses H as the
%   spacing between points in each direction.
%
%   [FX,FY] = GRADNAN(F,HX,HY), when F is 2-D, uses the spacing
...
%   HX, HY could also be Matrices with the same size then F.
%
%   [FX,FY,FZ] = GRADNAN(F), when F is a 3-D array, returns the
%   numerical gradient of F. FZ corresponds to dF/dz, the differences
%   in the z direction. GRADNAN(F,H), where H is a scalar, 
%   uses H as the spacing between points in each direction.
%
%   [FX,FY,FZ] = GRADNAN(F,HX,HY,HZ) uses the spacing given by
%   HX, HY, HZ. 
%
%   [FX,FY,FZ,...] = GRADNAN(F,...) extends similarly when F is N-D
...
%   $Revision: 5.11 $  $Date: 1997/04/08 05:22:44 $
...
for k = 1:ndim
   ...
   if n > 2
      ...
      g(2:n-1,:) = (f(3:n,:)-f(1:n-2,:))./h0;

    % Take forward differences if left NaN
    jj = find(    isnan(f(1:n-2,:))  & ...
                 ~isnan(f(2:n-1,:))  & ...
                 ~isnan(f(3:n-0,:))        );
          h0 = h(3:n,:) - h(2:n-1,:);
          if size(h0,2)==1
    ...
    g(jj+1) = (f(jj+2)-f(jj+1))./h0(jj);

    % Take back differences if right NaN 
    jj = find(   ~isnan(f(1:n-2,:))  & ...
                 ~isnan(f(2:n-1,:))  & ...
                  isnan(f(3:n-0,:))        ); 
          h0 = h(2:n-1,:) - h(1:n-2,:);      
